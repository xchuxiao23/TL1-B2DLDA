function f = Update_V_obj(x, y, v, a)
% Update_V_obj (GPU Version)
    [m, n, N] = size(x);
    u_labels = unique(y); 
    ...
    Sb_proj = pagefun(@mtimes, Sb_data, v);
    costs_Sb = (a + 1) .* abs(Sb_proj) ./ (a + abs(Sb_proj));
    numerator = sum(sum(costs_Sb, 1) .* reshape(num_eachclass, 1, 1, []));
    ...
    Sw_proj = pagefun(@mtimes, Sw_data, v);
